...
echo -e "${CYAN}  Setup Script — Linux / macOS${NC}"
echo ""
...
if command -v nvidia-smi &>/dev/null; then
  ...
  warn "Recommended: NVIDIA GPU with 8+ GB VRAM."
fi

# ─────────────────────────────────────────────────────────────────────────────
# STEP 3 — Download data from Google Drive
# ─────────────────────────────────────────────────────────────────────────────
step 3 "Downloading data (~10 GB from Google Drive)..."
...
if [ "$DATA_ALREADY_PRESENT" = false ]; then
  ...
  if ! command -v gdown &>/dev/null; then
    ...
    # Re-source PATH in case gdown was installed to ~/.local/bin
  ...
  # Detect zip structure: does it have a top-level 'data/' folder or not?
  FIRST_ENTRY=$(unzip -Z1 "${DATA_ZIP}" | head -1)
  if [[ "$FIRST_ENTRY" == data/* ]]; then
    # Zip contains data/ as top level — extract straight to REPO_ROOT
    unzip -q "${DATA_ZIP}" -d "${REPO_ROOT}"
  else
    # Zip contains turkey_data/, osrm/, etc. directly — extract into data/
...
# Create PostGIS init SQL if missing
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 5 — Verify critical data files
# ─────────────────────────────────────────────────────────────────────────────
step 5 "Verifying critical data files..."
...
# ─────────────────────────────────────────────────────────────────────────────
# STEP 6 — OSRM routing files
# ─────────────────────────────────────────────────────────────────────────────
step 6 "Checking OSRM routing files..."
...
echo -e "    • Ollama will download Gemma 4 models (~4–20 GB). This runs in the background."
...
echo -e "    • AI chat won't work until Gemma 4 models are fully downloaded."
echo ""
